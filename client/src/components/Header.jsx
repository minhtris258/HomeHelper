...
import {
  User, LogOut, Bell, LayoutDashboard, ClipboardList,
  UserCircle, Briefcase, Settings, PlusCircle, Home,
  Package, Search,
} from "lucide-react";
  ...
  const renderRoleActions = () => {
    switch (user?.role) {
      ...
      case "Homeowner":
        return (
          <>
            <Link to="/employer/profile" className="flex items-center px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-100 transition">
        ...
        <div className="max-w-[1440px] mx-auto px-4 py-3 flex justify-between items-center container">
          <div className="flex items-center gap-10">
            ...
            <nav className="hidden md:flex items-center gap-1">
              <NavLink to="/" className={navLinkClass}><Home size={18} /> Trang chủ</NavLink>
              <NavLink to="/jobs" className={navLinkClass}><Search size={18} /> Tìm việc làm</NavLink>
              <NavLink to="/packages" className={navLinkClass}><Package size={18} /> Gói dịch vụ</NavLink>
            </nav>
          </div>
            ...
            {user ? (
              <div className="flex items-center gap-4">
                <div className="relative group">
                  <button className="relative p-2 hover:bg-white/10 rounded-full transition group">
                    <Bell size={22} />
                    {unreadCount > 0 && (
                      <span className="absolute top-1 right-1 w-4 h-4 bg-red-500 text-[10px] flex items-center justify-center rounded-full border-2 border-[#3a1a7e] font-black animate-pulse">
                        {unreadCount}
                      </span>
                    )}
                  </button>
                  ...
                  <div className="absolute right-0 -mt-1 w-80 bg-white rounded-2xl shadow-2xl border border-slate-100 py-3 hidden group-hover:block z-[100] text-slate-800 animate-in fade-in slide-in-from-top-2">
                    <div className="px-4 py-2 border-b border-slate-50 flex justify-between items-center">
                      <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Thông báo mới</span>
                      {unreadCount > 0 && (
                        <button 
                    ...
                    <div className="max-h-80 overflow-y-auto">
                      {notifications.length > 0 ? (
                        notifications.map((n, i) => (
                          ...
                          >
                            <div className="flex justify-between items-start">
                                <p className={`text-sm leading-tight ${!n.isRead ? "font-black" : "font-medium"}`}>{n.title}</p>
                                {!n.isRead && <span className="w-2 h-2 bg-blue-500 rounded-full mt-1 shadow-sm"></span>}
                            </div>
                        ...
                        <p className="py-8 text-center text-xs text-slate-400 font-medium italic">Không có thông báo nào</p>
                      )}
                    </div>
                  </div>
                </div>
                ...
                <div className="relative group" onMouseEnter={() => setIsDropdownOpen(true)} onMouseLeave={() => setIsDropdownOpen(false)}>
                  <div className="flex items-center gap-3 pl-4 border-l border-white/20 cursor-pointer py-1">
                    <div className="w-10 h-10 bg-gradient-to-tr from-blue-600 to-blue-400 rounded-full flex items-center justify-center border-2 border-white/30 shadow-lg">
                      <span className="font-bold text-white text-lg">{user.fullName?.charAt(0).toUpperCase()}</span>
                    </div>
                    <div className="hidden sm:block text-left">
                      <div className="flex items-center gap-2">
                        <p className="text-sm font-bold leading-none uppercase tracking-tight">{user.fullName}</p>
                        {user.isPremium && <span className="bg-gradient-to-r from-yellow-400 to-orange-500 text-[9px] text-white px-2 py-0.5 rounded-full font-black uppercase">Premium</span>}
                      </div>
                      <p className="text-[10px] text-blue-300 uppercase mt-1 tracking-widest font-bold">
                        {user.role === "Admin" ? "Quản trị viên" : user.role === "Homeowner" ? "Chủ nhà" : "Người lao động"}
                      </p>
                    </div>
                  </div>
                  ...
                  {isDropdownOpen && (
                    <div className="absolute right-0 mt-0 w-60 bg-white rounded-2xl shadow-2xl py-3 border border-gray-100 animate-in fade-in slide-in-from-top-2 duration-200 text-gray-800">
                      <div className="px-4 py-2 mb-1 border-b border-gray-50 flex items-center justify-between">
                        <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Tài khoản {user.role}</p>
                        {user.isPremium && <span className="text-[10px] text-amber-600 font-black italic">Còn {user.daysLeft} ngày</span>}
                      </div>
                      {renderRoleActions()}
                      <div className="border-t border-gray-100 my-2"></div>
                      <button onClick={handleLogout} className="w-full flex items-center px-4 py-2.5 text-sm text-red-600 hover:bg-red-50 transition font-bold uppercase tracking-tighter">
              ...
              <div className="flex items-center gap-3">
                <div className="flex flex-col items-end text-right">
                  <span className="text-xs text-gray-300 font-medium">Chào mừng bạn</span>
                  <Link to="/login" className="font-bold text-sm hover:text-blue-300 transition uppercase tracking-tighter">Đăng ký / Đăng nhập</Link>
                </div>
                <div className="border-l border-white/20 h-8 mx-1"></div>
                <Link to="/register" className="bg-white/10 hover:bg-white/20 px-4 py-2 rounded-lg text-sm font-bold transition shadow-sm border border-white/5">Dành cho chủ nhà</Link>
              </div>
            )}
          </div>
        </div>
