  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-20 font-sans">
      <div className="pt-12 pb-24 text-center">
        <h1 className="text-white text-3xl font-bold uppercase tracking-tight">Kết quả tìm kiếm</h1>
        {location && (
          <p className="text-purple-200 mt-2 font-bold flex items-center justify-center gap-2">
            Khu vực: <span className="bg-white/20 px-3 py-1 rounded-lg text-white">{location}</span>
          </p>
        )}
      </div>
          ...
          <div className="flex-1">
            {loading ? (
              <div className="flex flex-col items-center justify-center py-20 bg-white rounded-[32px] border border-dashed">
                ...
                <p className="text-slate-400 font-bold italic">Đang lọc danh sách...</p>
              </div>
            ) : jobs.length > 0 ? (
              ...
              <div className="text-center py-24 bg-white rounded-[40px] border-2 border-dashed border-slate-100">
                ...
                <h3 className="text-xl font-black text-slate-800">Không có kết quả phù hợp</h3>
                <p className="text-slate-400 font-medium">Bạn có thể thử xóa bớt bộ lọc để thấy nhiều kết quả hơn.</p>
              </div>
            )}
          </div>
        </div>
