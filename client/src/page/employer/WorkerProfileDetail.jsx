...
const WorkerProfileDetail = () => {
  ...
  if (!data)
    return (
      <div className="p-20 text-center font-bold text-red-500">
        Không tìm thấy hồ sơ ứng viên này.
      </div>
    );
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 font-sans text-left">
      <button
        onClick={() => navigate(-1)}
        className="mb-6 flex items-center gap-2 text-slate-500 font-bold hover:text-blue-600"
      >
      ...
      <div className="bg-white rounded-[40px] shadow-sm border border-slate-100 p-8 md:p-12">
        <div className="flex flex-col md:flex-row gap-8 items-center md:items-start mb-10">
          <div className="w-32 h-32 bg-blue-600 rounded-[32px] flex items-center justify-center text-white text-4xl font-bold shadow-xl">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-3">
              <span className="flex items-center gap-1.5 bg-slate-50 text-slate-500 px-3 py-1 rounded-full text-xs font-bold">
                <MapPin size={14} />{" "}
                {data.profile.address || "Chưa cập nhật địa chỉ"}
              </span>
            ...
            <div className="mt-4 space-y-1">
              <p className="text-sm font-bold text-slate-600 italic">
                Liên hệ: {data.userInfo.phoneNumber}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid gap-8">
          ...
          <div className="bg-slate-50 rounded-[32px] p-6 border border-slate-100">
            <h3 className="flex items-center gap-2 font-bold text-slate-800 uppercase text-xs tracking-widest mb-4">
            ...
            <div className="flex flex-wrap gap-2">
              {data.profile.skills?.split(",").map((skill, index) => (
                <span
                  key={index}
                  className="bg-white border border-slate-200 px-4 py-2 rounded-xl text-sm font-bold text-slate-600 shadow-sm"
                >
                  {skill.trim()}
                </span>
              ))}
          ...
          {/* PHẦN KINH NGHIỆM */}
          <div>
            <h3 className="flex items-center gap-2 font-bold text-slate-800 uppercase text-xs tracking-widest mb-4 pl-2">
              <Briefcase size={18} className="text-blue-600" /> Kinh nghiệm làm
              việc
            </h3>
