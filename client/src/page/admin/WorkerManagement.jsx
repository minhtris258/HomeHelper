    ...
    <div className="p-8 text-left">
      <div className="flex justify-between items-center mb-8">
        <h2 className="text-2xl font-bold flex items-center gap-2">
          <Briefcase className="text-blue-600" /> QUẢN LÝ NGƯỜI GIÚP VIỆC (
          {workers.length})
        </h2>
        ...
        <table className="w-full">
          <thead className="bg-slate-50 ">
            <tr>
              ...
              <th className="p-5 text-xs font-bold uppercase text-slate-400 text-center">
                Hồ sơ
              </th>
              <th className="p-5 text-xs font-bold uppercase text-slate-400 text-center">
                Trạng thái
          ...
          <tbody>
            {loading ? (
              ...
              filteredWorkers.map((u) => (
                <tr
                  key={u.id}
                  className=" last:border-0 hover:bg-slate-50/50 transition-all"
                >
                  <td className="p-5">
                  ...
                  <td className="p-5 text-center">
                    {u.isApproved ? (
                      <span className="inline-flex items-center gap-1 text-green-600 font-bold text-[10px] uppercase bg-green-50 px-2 py-1 rounded-md">
                        <CheckCircle size={12} /> Đã duyệt
                      </span>
                    ) : (
                      <span className="inline-flex items-center gap-1 text-amber-500 font-bold text-[10px] uppercase bg-amber-50 px-2 py-1 rounded-md">
                        <XCircle size={12} /> Chờ duyệt
                      </span>
                    )}
                  </td>
                  <td className="p-5 text-center">
                    <span
                      className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase ${u.isLocked ? "bg-red-100 text-red-600" : "bg-green-100 text-green-600"}`}
                    >
