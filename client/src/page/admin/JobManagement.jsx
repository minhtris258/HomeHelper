...
const JobManagement = () => {
  ...
  const handleDeleteJob = async (id, title) => {
    if (
      !window.confirm(`Bạn có chắc chắn muốn xóa bài đăng: "${title}" không?`)
    )
      return;
    try {
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-slate-500 text-sm font-medium mt-1 italic">
            Kiểm soát nội dung và xử lý các tin đăng vi phạm quy định hệ thống.
          </p>
        </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 gap-5">
          {jobs.map((job) => (
            <div
              key={job.id}
              className="bg-white p-6 rounded-[32px] shadow-sm border border-slate-100 hover:shadow-xl hover:border-blue-200 transition-all flex flex-col md:flex-row justify-between items-start md:items-center gap-6 text-left"
            >
              <div className="flex-1 w-full">
                ...
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <span className="text-slate-400 text-xs font-bold uppercase italic">
                    ID Chủ nhà: #{job.ownerId}
                  </span>
                </div>
      ...
      >
        {selectedJob && (
          <div className="space-y-8 max-w-2xl text-left">
              ...
              <div className="bg-slate-50 p-5 rounded-[24px] border border-slate-100 flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="font-bold text-green-700 text-xl">
                    {selectedJob.salary.toLocaleString("vi-VN")} đ
                  </p>
                </div>
              </div>
              <div className="bg-slate-50 p-5 rounded-[24px] border border-slate-100 flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="font-bold text-slate-800 text-lg">
                    {selectedJob.workingTime || "Thỏa thuận"}
                  </p>
                </div>
              </div>
            </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="font-bold text-slate-700">
                      {selectedJob.genderReq || "Không yêu cầu"}
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="font-bold text-slate-700">
                      {selectedJob.ageReq || "Không yêu cầu"}
                    </p>
                  </div>
                </div>
            ...
            {/* Mô tả chi tiết */}
            <div>
              <h4 className="font-bold text-slate-800 mb-3 uppercase tracking-widest text-xs pl-1">
                Mô tả công việc
              </h4>
            ...
            <div className="flex items-center justify-between p-4 bg-blue-50/50 rounded-2xl border border-blue-100">
              ...
              <div className="flex items-center gap-2 text-xs font-bold text-slate-400">
                <CalendarDays size={14} /> Ngày đăng:{" "}
