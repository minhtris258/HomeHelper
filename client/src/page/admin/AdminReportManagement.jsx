...
const AdminReportManagement = () => {
  ...
  const handleBlockAction = async (jobId, userId) => {
    if (
      window.confirm(
        "CẢNH BÁO: Bạn có chắc chắn muốn XÓA BÀI và KHÓA tài khoản này không?",
      )
    ) {
      try {
      ...
      <div className="space-y-4">
            ...
            <div className="p-6 flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  onClick={() =>
                    setExpandedId(
                      expandedId === group.jobId ? null : group.jobId,
                    )
                  }
        ...
        {reportGroups.length === 0 && (
          <div className="py-20 text-center bg-white rounded-[40px] border-2 border-dashed border-slate-100">
            <p className="text-slate-400 font-bold italic uppercase">
              Hiện tại hệ thống không có tố cáo nào.
            </p>
          </div>
        )}
      </div>
