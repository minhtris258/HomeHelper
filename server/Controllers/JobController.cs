...
namespace server.Controllers
{
    ...
    public class JobController : ControllerBase
    {
        ...
        [HttpGet]
        public async Task<IActionResult> GetJobs([FromQuery] string? location, [FromQuery] string? keyword, [FromQuery] decimal? minSalary)
        {
            var query = _context.Jobs.AsQueryable();
            if (!string.IsNullOrEmpty(location)) query = query.Where(j => j.Location.Contains(location));
            if (!string.IsNullOrEmpty(keyword))
            {
                query = query.Where(j => j.Title.Contains(keyword) || j.ServiceType.Contains(keyword) || j.Description.Contains(keyword));
            }
            if (minSalary.HasValue) query = query.Where(j => j.Salary >= minSalary.Value);
